...
import { createMeetingLink } from "@/lib/metting-utils";
...
export class WorkspaceMeetingManager {
	...
	private meetingStore: MemoryWorkspaceMeetingStore =
	...
	private async sendNoticationToMembers(
		...
		const members = (await this.memberStore.listWorkspaceMembers(workspaceId)).filter(
			(m) => m.userId !== userId
		);
	...
	private async checkMeetingStatus(
		...
		const allMeetings = await this.meetingStore.queryMeetings({ status });
	...
	async createMeeting(input: TCreateMeetingInput): Promise<TeamMeetingDTO> {
		try {
			...
			const meetingType = isScheduledMeeting(input) ? "scheduled" : "active";

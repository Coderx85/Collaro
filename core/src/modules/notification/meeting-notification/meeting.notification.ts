...
import { INotification, INotificationDTO, INotificationStore } from "../interface";
...
export class MeetingNotification implements IMeetingNotification {  
  ...
  async createNotification(notification: TCreateNotificationInput): Promise<INotificationDTO> {
    return tryCatch({
      ctx: async () => {
        ...
        const dto: IMeetingNotificationDTO = {
          ...notification,
          id: notifacationId,
  ...
  async getNotificationsforUser(userId: IUserDTO["id"]): Promise<INotificationDTO[]> {
    return tryCatch({
      ctx: async () => {
        return await this.store.queryNotifications({ userId });
      }

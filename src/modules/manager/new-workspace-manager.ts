...
import { IMemberDTO, IMemberStore, IWorkspaceMemberManager, MemberStore } from "../member/index";
...
export class WorkspaceMemberManager implements IWorkspaceMemberManager {
  ...
  private async addMemberToWorkspace(workspaceId: IWorkspaceDTO["id"], userId: IUserDTO["id"]): Promise<IMemberDTO> {
  ...
  async joinWorkspace(params: Parameters<IWorkspaceMemberManager["joinWorkspace"]>[0]): ReturnType<IWorkspaceMemberManager["joinWorkspace"]> {
    const { workspaceId, userId } = params;
    await this.addMemberToWorkspace(workspaceId, userId);
    const workspace = await this.findWorkspaceById(workspaceId);
  ...
  async approveJoinRequest(requestId: TRequestId, approvedBy: TMemberId): Promise<IMemberDTO> {
    try {
      ...
      const newMember = await this.addMemberToWorkspace(request.workspaceId, request.userId);

...
export async function POST() {
  try {
    ...
    if (!user || user.length < 1 || !user[0].workspaceId) {
      ...
      return NextResponse.json({ message: 'You are not in a workspace' }, { status: 400 })
    }

    // Check if workspace exists
    const workspace = await db.select().from(workspacesTable).where(eq(workspacesTable.id, user[0].workspaceId)).execute()
    if(!workspace || workspace.length < 1) {
      console.log('Workspace does not exist')
      return NextResponse.json({ message: 'Workspace does not exist' }, { status: 400 })
    }
    ...
    if (workspace[0].createdBy === user[0].id) {
      ...
      await db.delete(workspacesTable).where(eq(workspacesTable.id, user[0].workspaceId)).execute()
    ...
    return NextResponse.json({ message: 'Successfully left workspace' }, { status: 200 })
  } catch (error: unknown) {
    console.error(error)
    return { status: 500, body: { message: `Failed to execute Remove Operation: ${error}` } }
  }
}
...
export async function GET(): Promise<
  NextResponse<APIResponse<AnalyticsResponse>>
> {
  try {
    ...
    const totalMeetings = analytics.reduce(
      (sum, workspace) => sum + workspace.totalMeetings,
      0,
    );
    const totalMembers = analytics.reduce(
      (sum, workspace) => sum + workspace.totalMembers,
      0,
    );
